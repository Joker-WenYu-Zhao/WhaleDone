...
- **`path`'s filename segment MUST be `encodeURIComponent`-encoded by the caller.** Supabase Storage rejects non-ASCII object keys, so a raw Chinese (or any non-ASCII) filename fails. Encode the filename only, never the whole path — encoding `/` breaks the folder structure:

```tsx
const path = `${userId}/${encodeURIComponent(originalName)}`; // "报表.jpg" -> "%E6%8A%A5%E8%A1%A8.jpg"
```

  This is what makes Chinese filenames work; do not "fix" such uploads by transliterating or dropping the original name. `data.path` comes back still encoded — pass it to `getPublicUrl` as-is and only `decodeURIComponent` for display.
