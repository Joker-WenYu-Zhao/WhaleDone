...
- **Filenames MUST be `encodeURIComponent`-encoded before upload.** Supabase Storage rejects non-ASCII object keys, so a raw Chinese (or any non-ASCII) filename fails. Encode the filename segment only — never the whole path, or the `/` separators get escaped too:

```ts
const safeName = encodeURIComponent(file.name); // "报表.jpg" -> "%E6%8A%A5%E8%A1%A8.jpg"
```

  This is what makes Chinese filenames work; do not "fix" such uploads by transliterating or dropping the original name. The returned `data.path` stays encoded — pass it to `getPublicUrl` as-is and only `decodeURIComponent` for display.
