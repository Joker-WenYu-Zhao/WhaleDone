...
- **Filenames MUST be `encodeURIComponent`-encoded before upload.** Supabase Storage rejects non-ASCII object keys, so a raw Chinese (or any non-ASCII) filename fails. Encode the filename segment only — never the whole path, or the `/` separators get escaped too. This is what makes Chinese filenames work; do not "fix" such uploads by transliterating or discarding the original name.
- Web: upload directly using `File` / `Blob`:

```ts
// encode the filename segment only, NOT the whole path
const safeName = encodeURIComponent(file.name); // "报表.jpg" -> "%E6%8A%A5%E8%A1%A8.jpg"
...
- `data.path` comes back still encoded. Pass it to `getPublicUrl` as-is; only `decodeURIComponent` when displaying the name to a user.
