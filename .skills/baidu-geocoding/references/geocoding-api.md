...
| Endpoint | `GET https://app-dzw9lby2igw1-api-GaDwZ0j3erOY-gateway.appmiaoda.com/geocoding/v3/` |
| 认证模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>` |
| third_part_domain | `app-dzw9lby2igw1-api-GaDwZ0j3erOY-gateway.appmiaoda.com` |
| 计费 | 免费，但会统计调用次数 |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dzw9lby2igw1-api-GaDwZ0j3erOY-gateway.appmiaoda.com/geocoding/v3/?${params.toString()}`,
    {
      method: "GET",
      headers: {
