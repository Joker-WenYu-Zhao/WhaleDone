...
| 麦克风 | `navigator.mediaDevices.getUserMedia({ audio: true })` | 用户手势触发 + 系统弹窗授权；仅能实时处理，**无法播放录音**（无音频播放能力） |
| 选择图片 / 拍照 | `<input type="file">` | 系统选择器接管，**仅能选图片和视频**（无论 `accept` 如何设置） |

**音视频播放不可用**：`<audio>` / `<video>` 在小工具里无法使用 —— 媒体文件扩展名不在打包白名单内（仅 `html css js json png jpg jpeg gif webp svg woff woff2`），外链媒体与 `data:` / `blob:` 媒体也被容器 CSP 禁止。需要音效 / 背景音乐 / 视频播放的设计必须去掉，改用动画、高亮等视觉反馈替代。详见 [zip-artifact-spec.md](./zip-artifact-spec.md) §2。
...
| 音视频播放 | `<audio>` / `<video>`、`new Audio()`：媒体文件不在打包白名单，外链与 `data:` / `blob:` 媒体也被禁 | 移除音效 / 背景音乐 / 视频播放，改用动画、高亮等视觉反馈 |
...
<audio> / <video> / new Audio(   ·   引用 .mp3 / .wav / .m4a / .ogg / .mp4 / .mov / .webm
    —— 媒体文件不在打包白名单，音视频播放整体不可用
@font-face 引用 .ttf / .otf / .eot   —— 字体只允许 .woff / .woff2
```
