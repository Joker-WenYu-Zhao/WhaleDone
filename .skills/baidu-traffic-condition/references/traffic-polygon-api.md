...
| Endpoint | `GET https://app-dzw9lby2igw1-api-BYdwQ5e51blL-gateway.appmiaoda.com/traffic/v1/polygon` |
| Auth | `platform_managed`（`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}`） |
| Content-Type | `application/json`（GET 请求，参数通过 Query String 传递） |
| third_part_domain | `app-dzw9lby2igw1-api-BYdwQ5e51blL-gateway.appmiaoda.com` |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dzw9lby2igw1-api-BYdwQ5e51blL-gateway.appmiaoda.com/traffic/v1/polygon?${params.toString()}`,
    {
      method: "GET",
      headers: {
