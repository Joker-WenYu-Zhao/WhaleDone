...
| Endpoint | `GET https://app-dzw9lby2igw1-api-rLobR3D3dbg9-gateway.appmiaoda.com/traffic/v1/road` |
| Auth | `platform_managed`（`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}`） |
| Content-Type | `application/json`（GET 请求，参数通过 Query String 传递） |
| third_part_domain | `app-dzw9lby2igw1-api-rLobR3D3dbg9-gateway.appmiaoda.com` |
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dzw9lby2igw1-api-rLobR3D3dbg9-gateway.appmiaoda.com/traffic/v1/road?${params.toString()}`,
    {
      method: "GET",
      headers: {
